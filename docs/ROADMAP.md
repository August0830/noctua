...
| 2.4 | 实现决策追溯链（每个动作 → 来源 LoRA/技能/历史） | P0 | `[x]` | 2.3 |
...
| 2.9 | 端到端运行 Rome 推理场景（人类在环） | P1 | `[x]` | 2.7 |
...
| ID | 任务 | 优先级 | 状态 | 依赖 |
|----|------|--------|------|------|
| 3.1 | 实现 VerifierActor 质量评分（基于 Rome 设计） | P0 | `[x]` | 2.3 |
| 3.2 | 实现人类检查点操作捕获：确认 / 修改 / 拒绝 / 补充理由 | P0 | `[x]` | 2.4 |
| 3.3 | 实现操作→训练数据转换器（确认→正样本、修改→DPO pair、拒绝→负样本） | P0 | `[x]` | 3.2 |
